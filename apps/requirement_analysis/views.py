                        ...
                        def execute_task():
                            try:
                                ...
                                try:
                                    ...
                                    logger.info(f"开始生成测试用例，任务ID: {task.task_id}")
                                    logger.info(f"编写模型: {task.writer_model_config.name}")
                                    logger.info(f"编写提示词: {task.writer_prompt_config.name}")
                                    ...
                                    try:
                                        ...
                                        # 检查生成结果
                                        if not generated_cases or len(generated_cases.strip()) == 0:
                                            raise Exception("AI模型返回了空内容")
                                        
                                        logger.info(f"测试用例生成成功，内容长度: {len(generated_cases)}")
                                        logger.info(f"生成内容预览（前200字符）: {generated_cases[:200]}")
                                        ...
                                        logger.error(f"生成测试用例失败: {gen_error}", exc_info=True)
                                        raise Exception(f"测试用例生成失败: {str(gen_error)}")
                                ...
                                task.generation_log = f"任务执行失败: {str(e)}"
                                task.save()
